...
已获得 npm 安装包的用户可执行 `npm install -g /路径/novel-code-0.3.0.tgz`。已解压源码的用户可以在源码目录执行 `node bin/novel-code.js`，或运行 `npm install -g .`。
...
输入 `/work` 或按 F2 开启：保持代码助手外观和当前阅读页，随意敲字母、数字、中文、空格、回车、退格等都不会显示输入，也不会触发 q、b、c 等阅读快捷键。`/` 专门保留为命令入口。

- ↓ / → / PageDown 下翻，↑ / ← / PageUp 上翻，沿用 `/step` 行数。
- 按 `/` 唤起命令框，可输入 `/step 5`、`/auto 10`、`/chapters` 等。命令执行完或按 Esc 返回后，继续保持工作模式；不用先按 F2 解锁。
- `/work off` 退出工作模式。也保留 F2 开关；部分 Mac 键盘需要按 Fn+F2。
- Esc 仍可隐藏 / 恢复正文；Ctrl+C 或 Ctrl+D 保存退出。
- 支持括号粘贴的终端中，工作模式下直接粘贴的文字会被忽略，包括其中的 `/`；先按 `/` 打开命令框后可正常粘贴路径或命令参数。
...
输入 `/auto 10` 开始每 10 秒翻动一次，范围为 1–3600 秒；`/auto on` 默认每 10 秒，`/auto off` 停止，`/auto` 查看当前状态。翻动行数沿用 `/step`，例如先输入 `/step 5` 再输入 `/auto 10`，就是每 10 秒下移 5 行。普通阅读和工作模式都可使用，底部显示自动翻页状态。

输入命令、打开目录或搜索等菜单、隐藏正文、窗口过小时都会暂停。返回阅读或手动翻页后，会重新等待完整的设定间隔；工作模式里被忽略的乱打字不影响计时。到书末、成功打开另一本书或退出时停止，下次启动不会自动开启。
...
| `/work`、`/work off` | 开启 / 退出乱打字模式；模式内按 `/` 可临时输入命令 |
| `/step 5` | 每次翻动 5 个显示行（1–200，最多一屏），自动保存 |
| `/step`、`/step auto` | 查看翻动设置 / 恢复整页 |
| `/auto 10`、`/auto on` | 按指定秒数自动翻动；on 默认为 10 秒，沿用 `/step` 行数 |
| `/auto`、`/auto off` | 查看自动翻页状态 / 停止 |
| `/help`、`/quit` | 帮助 / 保存退出 |
